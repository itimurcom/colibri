...
//			TAB."</form>";
	...
	public function _view_fields()
		...
		foreach ($this->fields_xml as $key=>$row)
			...
			$row['ed_key']		= $key;
			$row['class']		= !isset($row['class']) ? $this->class : $row['class'];
...
//						echo print_rr($row); die;
			...
			$class_str	= !empty($row['class'])	? " {$row['class']}" : 
			...
			$code.= !mempty($label_zone, $editor_zone, $value_zone)
				? 	$error_str.
					...
					$label_zone.
					$editor_zone.
					TAB."<div class=\"value boxed{$compact}{$full}{$class_str}\">".$value_zone.TAB."</div>".
					TAB."</div>"
				: NULL;
...
//				"<span class='red'>{$data['kind']}".($data['ed_key']+1)."</span>".
		...
		foreach ($this->fields_xml as $key=>$row)
			...
			$row['class']		= !isset($row['clas']) ? $this->class : $row['class'];
	...
	public function _view_buttons() {
		...
		if (is_array($this->buttons_xml)) {
			...
			foreach ($this->buttons_xml as $key=>$row) {
				// подготовим настройки для поля
				$row['table_name']	= $this->table_name;
				$row['form_id'] 	= !isset($row['form_id']) ? $this->form_id : $row['form_id'];
				$row['rec_id']		= $this->rec_id;
				$row['ed_key']		= $key;
...
//..............................................................................
function _arguments($params, $defaults_arr, &$result)
	{
	$result_arr = NULL;
