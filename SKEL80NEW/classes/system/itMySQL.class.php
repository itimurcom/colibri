...
class itMySQL
		...
		if (defined('SKIP_SQL')===false)
			...
			try
				...
				$this->db = mysqli_connect($db_server, $db_user, $db_pass, $db_name, $db_port);
					...
					{
					echo 'Error during connection to database :'.$e->getMessage();
					die;
					}
			...
			if ($this->db)
				...
				$this->db_server	= $db_server;
					...
					echo "Something wrong with DB connection.".BR.mysqli_connect_error(); die;
				}
			}
		}
	...
	// возвращает id последней записи в базе без объекта
	//..............................................................................
	static function _last_inserted_id($table_name=NULL, $db_prefix=DB_PREFIX, $database=CMS_DB_NAME) {
		$db = new itMySQL();
		$result = $db->get_last_inserted_id($table_name, $db_prefix, $database);
		unset($db);
		return $result;
		}
	...
	//..............................................................................
	public function get_last_inserted_id($table_name=NULL, $db_prefix=DB_PREFIX, $database=CMS_DB_NAME) {
		$request = itMySQL::_request("SELECT LAST_INSERT_ID() as lastid FROM {$db_prefix}{$table_name}");
		return (isset($request[0]) AND isset($request[0]['lastid'])) ? $request[0]['lastid'] : false;
		}
	//..............................................................................
	...
	public function update_value_db($table_name=NULL, $rec_id=NULL, $value=NULL, $field='')
		...
		$value = !is_null($value) ?  "'".addslashes($value)."'" : 'NULL';
		...
		if (is_array($values_arr))
			...
			foreach ($values_arr as $key=>$row)
				...
				if (is_array($row))
					{
					if (is_array($row))	{
						$row = json_encode($row, JSON_HEX_APOS | JSON_HEX_QUOT);
						} else $row=NULL;
					}					
