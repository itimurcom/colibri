...
class itFeed
	...
	public function __construct($options=NULL)
		...
		$this->WASRESET		= false;
		$this->COUNTALL		= NULL; 								// начало
		$this->LIMIT		= ready_val($options['limit'], $this->MAXINBLOCK * 2);
			...
			$this->COUNTALL = $this->onefield ? 100 : (is_array($this->request) ? count($this->request) : 0);
				...
				$this->query = (is_null($this->sql))
					...
					?	"SELECT {$this->fields} FROM {$this->prefix}{$this->table_name} ".
						...
						((!is_null($this->position) AND !$this->onefield) ? " LIMIT {$this->position}".((intval($this->LIMIT)>0) ? ",".$this->LIMIT : "") : "")
