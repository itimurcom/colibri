...
- fixed PHP 8.2+ startup deprecations from `itFeed` dynamic runtime properties
- made `isJson()` NULL-safe to stop repeated `json_decode(null)` deprecations
- normalized DB JSON decode path in `itMySQL` to avoid passing NULL/empty strings into `json_decode()`

## M0 / P9 mail forms dynamic-properties hotfix
- Added explicit runtime properties in `itModal`, `itButton`, and `itForm2` to remove PHP 8.2+ dynamic-property deprecations visible in the admin mail section and price calculator.
- Fixed `itModal::set_animation()` to write to the declared animation field instead of creating a dynamic property.
- Normalized `itForm2` button-array writes to `buttons_xml` to keep button editor/runtime state aligned.
